...
use warnings;
use strict;
die "USAGE:\n $0 < transposon file><parsed_genome_annotation_file> <statistic_output_file> " unless (@ARGV==3);
...
while(<TE>){
   ...
   if(/Nested/){
         ...
         my $start = $TE[2];
         my $end = $TE[3];
         my $gene = "grep -E '$TE[9]' $genome_anno > bar_$TE[9].gff3";
            system($gene);
